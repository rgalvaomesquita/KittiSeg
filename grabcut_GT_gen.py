...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 5:
        ...
        filename = sys.argv[2] # for drawing purposes
        output_dir = sys.argv[3]
        classLabel = sys.argv[4]
    else:
    ...
    while(1):
        ...
        if k == 27:         # esc to exit
            ...
            res = np.ones(output.shape[:2],np.uint8)*255
            gray_image = cv.cvtColor(output, cv.COLOR_BGR2GRAY)
            th, res = cv.threshold(gray_image, 1, 255, cv.THRESH_BINARY)
            res = cv.cvtColor(res,cv.COLOR_GRAY2BGR)
            if classLabel == 'blue':
                res[np.where((res == [255,255,255]).all(axis = 2))] = BLUE
            elif classLabel == 'green':
                res[np.where((res == [255,255,255]).all(axis = 2))] = GREEN
            elif classLabel == 'red':
                res[np.where((res == [255,255,255]).all(axis = 2))] = RED
            else:
                res[np.where((res == [255,255,255]).all(axis = 2))] = WHITE
